...
function Constellation(){
    ...
    const filltered = stars.filter(st => st.season === season || st.season === "通年");
    const Refcanvas = useRef(null);
    const refBgstars = useRef([]);
    useEffect(() => {
        const canvas = Refcanvas.current;
        const pen = canvas.getContext("2d");
        canvas.style.background ="black";
        const Newstars =refBgstars.current
        ...
        async function searchLocation() {
            try{
                const loca = await getlocation();
                const lat = loca.coords.latitude;
                const lng = loca.coords.longitude;
                const Date = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`);
                const result = await Date.json();
                setLocation(result.address?.town || result.address?.city ||"現在地")
            }catch(error){
    ...
    useEffect(() => {
        ...
        currentStar.stars.forEach(st => {
            pen.beginPath();
            pen.arc(st.x,st.y,st.radius,0,Math.PI * 2);
            pen.fillStyle = st.color;
            pen.fill();
        });
    ...
    return(
        <div>
            ...
            {filltered.map(st => (
                <div key={st.name}>
                    <p>{st.name}</p>
                    {currentStar === st && <p>{st.description}</p>} 
                    <button onClick={() => setcurrentStar(st === currentStar?null:st)}>特徴表示</button>
                </div>
            ))}
        </div>
    )
